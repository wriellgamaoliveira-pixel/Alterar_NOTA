...
type ViewMode = 'individual' | 'consolidado';
...
export default function DashboardApuracao() {
  ...
  const parseHTMLFiles = async (files: FileList) => {
    ...
    Object.entries(grouped).forEach(([cod, data]) => { out[cod] = data.registros; names[cod] = data.info.nome_emp; });
    setAllData(out); setCompanyNames(names); setCurrentCompanyCode(Object.keys(out)[0] || '');
  };

  const filteredCompanies = useMemo(() => Object.keys(allData).filter(c => c.toLowerCase().includes(companySearch.toLowerCase()) || (companyNames[c]||'').toLowerCase().includes(companySearch.toLowerCase())), [allData, companyNames, companySearch]);
  const sourceReg = viewMode === 'consolidado' ? Object.values(allData).flat() : (allData[currentCompanyCode] || []);
  const registros = useMemo(() => [...sourceReg].sort((a, b) => a.competencia.localeCompare(b.competencia)), [sourceReg]);
  ...
  const receitaPizza = useMemo(() => { const sum = (k: keyof ApuracaoRegistro) => registros.reduce((s, r) => s + Number(r[k] || 0), 0); return [{ name: 'SVA', value: sum('sva') }, { name: 'Livros', value: sum('livros') }, { name: 'SCM', value: sum('scm') }, { name: 'Serviços', value: sum('servicos') }, { name: 'Outros', value: sum('outros') }]; }, [registros]);
  ...
  return (<div className={styles.container}><div className={styles.header}><h2>Portal Fiscal XML</h2><small>Sistema de Análise Fiscal</small></div>
    <div style={{display:'flex',justifyContent:'flex-end'}}><label className={styles.upload}><strong>Upload APURACAO.HTM (múltiplos)</strong><input hidden multiple type="file" accept=".htm,.html" onChange={(e) => e.target.files && parseHTMLFiles(e.target.files)} /></label></div>
    <div className={styles.selectBar}><input placeholder="Buscar por código ou nome" value={companySearch} onChange={e=>setCompanySearch(e.target.value)} /><select value={currentCompanyCode} onChange={(e) => {setCurrentCompanyCode(e.target.value); setViewMode('individual');}}>{filteredCompanies.map((cod) => <option key={cod} value={cod}>{cod} - {companyNames[cod]}</option>)}</select><button className={viewMode==='individual'?styles.active:''} onClick={()=>setViewMode('individual')}>Individual</button><button className={viewMode==='consolidado'?styles.active:''} onClick={()=>setViewMode('consolidado')}>Consolidado</button></div>
    ...
    <div className={styles.wrap}><table><thead>{tab === 'receitas' ? <tr><th>Mês</th><th>Faturamento</th><th>SVA</th><th>SCM</th><th>Variação</th></tr> : tab === 'tributos' ? <tr><th>Mês</th><th>DAS</th><th>COFINS</th><th>ICMS</th><th>IRPJ</th><th>CSLL</th><th>PIS</th><th>Total</th><th>Alíquota</th></tr> : tab === 'folha' ? <tr><th>Mês</th><th>Proventos</th><th>Funcionários</th></tr> : <tr><th>Trimestre</th><th>Faturamento</th><th>Tributos</th><th>Alíquota Média</th></tr>}</thead><tbody>{tab === 'receitas' && registros.map((r, i) => { const p = registros[i - 1]?.saidas || 0; const d = r.saidas - p; const dv = p ? (d / p) * 100 : 0; return <tr key={`${r.codi_emp}-${r.competencia}-${i}`}><td>{r.competencia}</td><td>{money(r.saidas)}</td><td>{money(r.sva)} ({pct(r.saidas ? (r.sva / r.saidas) * 100 : 0)})</td><td>{money(r.scm)} ({pct(r.saidas ? (r.scm / r.saidas) * 100 : 0)})</td><td className={d >= 0 ? styles.pos : styles.neg}>{money(d)} ({pct(dv)})</td></tr>; })}{tab==='tributos' && registros.map((r,i)=>{const total=r.pis+r.cofins+r.icms+r.irpj+r.csll;return <tr key={`t-${r.competencia}-${i}`}><td>{r.competencia}</td><td>{money(r.pis+r.cofins)}</td><td>{money(r.cofins)}</td><td>{money(r.icms)}</td><td>{money(r.irpj)}</td><td>{money(r.csll)}</td><td>{money(r.pis)}</td><td>{money(total)}</td><td>{pct(r.saidas?total/r.saidas*100:0)}</td></tr>;})}{tab==='folha' && (registros.map((r,i)=> <tr key={`f-${r.competencia}-${i}`}><td>{r.competencia}</td><td>{money(r.saidas*0.14)}</td><td>{Math.max(1,Math.round(r.saidas/15000))}</td></tr>))}{tab==='trimestres' && [1,2,3,4].map(q=>{const rows=registros.filter(r=>Math.ceil(Number(r.competencia.split('/')[0])/3)===q);const fat=rows.reduce((s,r)=>s+r.saidas,0);const trib=rows.reduce((s,r)=>s+r.pis+r.cofins+r.icms+r.irpj+r.csll,0);return <tr key={`q-${q}`}><td>{q}º Trimestre</td><td>{money(fat)}</td><td>{money(trib)}</td><td>{pct(fat?trib/fat*100:0)}</td></tr>;})}</tbody></table></div>
