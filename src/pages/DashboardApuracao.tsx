...
const per=(n:number)=>`${(Number.isFinite(n)?n:0).toFixed(1)}%`;
...
export default function DashboardApuracao(){
  ...
  const totalClientes=view==='consolidado'
    ? Object.keys(all).reduce((sum,cod)=>sum+(clientes[cod]||0),0)
    : (clientes[current]||0);
  const ticketMedio=totalClientes>0?fat/totalClientes:0;

  const perCompany=useMemo(()=>Object.keys(all).map(cod=>{
    const r=all[cod]||[];
    const faturamento=r.reduce((s,i)=>s+i.saidas,0);
    const tributos=r.reduce((s,i)=>s+i.pis+i.cofins+i.icms+i.irpj+i.csll,0);
    return {cod,nome:names[cod]||cod,faturamento,tributos,clientes:clientes[cod]||0};
  }).sort((a,b)=>b.faturamento-a.faturamento),[all,names,clientes]);
  ...
  return <div className={styles.wrap}><div className={styles.header}><div><h1>Dashboard de Apuração</h1><p>Sistema de Análise Financeira e Tributária</p></div><div><button className={styles.btnLight} onClick={()=>{setAll({});setNames({});setCurrent('');setFilesLoaded([]);setClientes({})}}>Reiniciar</button><button className={styles.btnGreen} onClick={()=>fileRef.current?.click()}>Carregar Arquivo(s)</button></div></div>
    ...
    <div className={styles.cards}><Card t='TICKET MÉDIO' v={totalClientes>0?brl(ticketMedio):'N/D'} c='blue'/><CardInput t='TOTAL CLIENTES' value={totalClientes} disabled={view==='consolidado'} onChange={(v)=>current&&setClientes(prev=>({...prev,[current]:v}))} c='green'/><Card t='LIQUIDEZ' v={rows.length?per((fat-trib)/fat*100):'N/D'} c='teal'/><Card t='FATURAMENTO ACUMULADO' v={brl(fat)} c='purple'/><Card t='TOTAL TRIBUTOS' v={brl(trib)} c='orange'/><Card t='TOTAL FUNCIONÁRIOS' v={rows.length?String(Math.round(fat/15000)):'N/D'} c='red'/></div>

    {view==='consolidado'&&<div className={styles.tableBox}><table><thead><tr><th>Código</th><th>Empresa</th><th>Faturamento</th><th>Tributos</th><th>Clientes</th><th>Ticket Médio</th></tr></thead><tbody>{perCompany.map((e)=><tr key={e.cod}><td>{e.cod}</td><td>{e.nome}</td><td>{brl(e.faturamento)}</td><td>{brl(e.tributos)}</td><td>{e.clientes||'N/D'}</td><td>{e.clientes?brl(e.faturamento/e.clientes):'N/D'}</td></tr>)}</tbody></table></div>}
...
function CardInput({t,value,disabled,onChange,c}:{t:string;value:number;disabled:boolean;onChange:(v:number)=>void;c:string}){return <div className={`${styles.card} ${styles[c]}`}><small>{t}</small><input className={styles.cardInput} type='number' min={0} value={value||''} disabled={disabled} onChange={e=>onChange(Number(e.target.value||0))} placeholder={disabled?'Total automático no consolidado':'Digite manualmente'} /></div>}
